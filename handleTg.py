    ...
    def run(self):
        ...
        while True:
            ...
            if data is None:
                ...
                if int(user_tg_id) == -999:
                    bot_url = helpp.get_bot_url(group_tg_id, 2)
                    
                    net.deleteMessageOne(bot_url, group_tg_id, data["message_tg_id"])

                    continue
                
                if typee == "sendSpecialWelcome":
                    bot_url = helpp.get_bot_url(group_tg_id, 2)
                    ...
                    net.send_btn_and_delete_last(bot_url, group_tg_id, data["text"], btns, "sendSpecialWelcome")
                ...
                elif typee == "cancelRestrict":
                    flag, description = net.cancelRestrictChatMemberWrap(bot_url, group_tg_id, user_tg_id)
                    
                    print("cancelRestrict %s %s" % (flag, description))
                    
                    if flag:
                        db.user_group_new_update(group_tg_id, user_tg_id, 2, 1, 1, 1)
                elif typee == "deleteAll":
                    # 删除所有信息
                    if "message_tg_id" in data:
                        print("==> delete_one %s %s %s %s" % (group_tg_id, user_tg_id, data["message_tg_id"], reason))
